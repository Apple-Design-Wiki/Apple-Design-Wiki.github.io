...
当比尔与拉里要为 Lisa设计的使用者交互界面有了初步定义，这两人发展出非常好的合作默契，并以类似于当初拉里与蒂姆·莫特一起研发Gypsy时，那种夜以继日的模式工作着;比尔晚上工作，拉里白天工作。晚上，比尔针对使用者界面的概念设计出原型，并粗略写了些足以应付测试的程序代码;次日白天，由拉里进行使用者测试。当时要找目标测试者非常容易，因为苹果公司多数新进员工都还没用过计算机。拉里在一天结束后，将结果书面呈给比尔，让他知道测试结果与收获然后一起进行头脑风暴，决定下一步如何进行。然后晚上写程序，白天将测试的流程再跑一次。这样的工作状态维持了数周之久，直到产品特性都明确为止。

后来，他们有了比较常态化的分工。因为小组成员增加，有不同的人可以写程序，而一有成果就交给拉里进行测试。拉里针对测试写评估与建议后，再进行小组讨论，弄清发展细节。拉里常被视为拖延进度的人，因为他总是希望可以作出改变，不过他的坚持也让产品的发展落实了容易操作的理念。Lisa共花了3年时间研发从1979年进行到1983年，营销部的人质疑这个该进行6个月的项目怎么会耗时3年，但工程师认为，要研发出一个新的使用者界面、一个新的操作系统与5项前所未有的应用程序，3年并不算久。

比尔与拉里在合作初期即构思出要在屏幕上方放个下拉式菜单，类似我们现在看到的那样。不过由于最早参与Lisa研发的人有很多是从惠普(HP)公司过来的，他们对于硬件设计已有想法，像是要用一个点阵式显示器，显示器下方有一个类似惠普机种的矩形按键列，至于快捷键，键盘上有一列非标签式的设计，屏幕上方出现的则是标签式的;按下那些按键，就能在屏幕上看见那些按键的功能说明。它还有一个菜单分级，当你按下按键，所有标签会换成下个层级的。拉里不同意这个做法，因为惠普机种带给使用者的唯一贡献，是可以鼠标点选功能，而不是利用快捷键。
...
![alt text](./v2-3a61236adef57145a98f0916387e175b_1440w.webp)
